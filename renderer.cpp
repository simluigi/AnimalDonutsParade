...
// ヘッダーファイルの性的変数を再宣言
// redeclaring static variables from header file
...
void CRenderer::Init()
{
	...
	// 深度ステンシル用テクスチャー作成	
	// Create Depth Stencil (for texture)
	...
	rd.CullMode = D3D11_CULL_BACK;		//erase back (unseen portion)
	...
	// サンプラーステート設定（貼り付けの設定）
	// Sampler State (desc); texture mapping settings
	...
	// vertex and pixel shaders moved to respective functions below
	// see CreateVertexShader, CreatePixelShader below
	CRenderer::CreateVertexShader(&m_VertexShader, &m_VertexLayout, "vertexShader.cso");
	CRenderer::CreatePixelShader(&m_PixelShader, "pixelShader.cso");	// default

	// 定数バッファ生成	
	// Create Const buffer (matrix, material, lighting)
...
// マトリックス設定
// Matrix Settings (world, view, projection)
...
// ライト設定 light.cppで呼び出す
// Set Light; to be called by light class (light.cpp)
...
// 2020.09.23 Replace default Shader creation in source file
// 頂点シェーダ生成
void CRenderer::CreateVertexShader(ID3D11VertexShader** VertexShader, ID3D11InputLayout** VertexLayout, const char* FileName)
{
	...
	{
		//follow semantics POSITION, NORMAL etc
		// POSITION + 0 = POSITION0 used in common.hlsl
		// R32 G32 B32: 8byte width per color
