...
void CModel::Load( const char *FileName )
{
	...
	// 各バッファー配列を削除
	// Uninit all three buffer arrays
...
void CModel::LoadObj( const char *FileName, MODEL *Model )
{
	...
	//2020.07.03
	//修正：fopenに対するfcloseを追加しました。 (375行目）
	FILE *file;
	...
	while( true )
	{
		...
		else if( strcmp( str, "f" ) == 0 )
		{
			...
			do
			{
				...
				{
					// テクスチャ座標が存在しない場合もある
					// take into consideration the possibility of nonexistent texture coordinates
					s = strtok( NULL, "/" );
	...
	// 修正 fopen -> fclose; ファイルの読み込み終了処理
	// fclose file (file read process completed)
	fclose(file);
...
void CModel::LoadMaterial( const char *FileName, MODEL_MATERIAL **MaterialArray, unsigned int *MaterialNum )
{
	...
	// 2020.07.03
	// 修正：fopenに対するfcloseを追加しました。 (501行目）
	FILE *file;
	file = fopen( FileName, "rt" );	
	assert(file);
