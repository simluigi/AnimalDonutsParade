...
{ 
    ...
    ////specular
    //float3 reflection = reflect(Light.Direction.xyz, inNormal.xyz);
    //float specular = dot(normalize(reflection), normalize(Light.Direction.xyz));
    //if (specular > 0.0f)
    //{
	...
	if (Light.Enable)
	{
        ...
        ////specular
        //outDiffuse = saturate(outDiffuse + specular);
